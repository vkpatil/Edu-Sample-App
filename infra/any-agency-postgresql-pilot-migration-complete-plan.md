# Any Agency PostgreSQL Migration - Complete Pilot Plan

## Document Information

- **Agency:** Any Agency
- **Workload:** Sample Application (Pilot - Dev)
- **Source:** On-prem / existing PostgreSQL
- **Target:** Azure Database for PostgreSQL - Flexible Server
- **Migration Type:** Offline (Dump and Restore) for Pilot
...
This document provides a complete, end-to-end migration plan for executing a PostgreSQL pilot migration. The pilot is intentionally designed to be:

- Low risk
- Repeatable
- Observable
- Aligned to the target network model
...
- **Method:** Offline dump and restore
- **Why:**
...
### Infrastructure and Network Team
...
```bash
pg_dumpall -r --no-role-passwords \
  -h <source_host> \
  -U <source_admin_user> \
  > roles.sql
```
...
| Insufficient storage | Allocate approximately 25% buffer for WAL |
